...
/// struct for typed errors of method [`v1_users_id_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum V1UsersIdGetError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`v1_users_me_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum V1UsersMeGetError {
    UnknownValue(serde_json::Value),
}


/// Returns a paginated list of user objects for a workspace
pub async fn v1_users_get(configuration: &configuration::Configuration, notion_version: Option<&str>) -> Result<serde_json::Value, Error<V1UsersGetError>> {
...
pub async fn v1_users_id_get(configuration: &configuration::Configuration, id: &str, notion_version: Option<&str>) -> Result<serde_json::Value, Error<V1UsersIdGetError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<V1UsersIdGetError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}

pub async fn v1_users_me_get(configuration: &configuration::Configuration, notion_version: Option<&str>) -> Result<serde_json::Value, Error<V1UsersMeGetError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<V1UsersMeGetError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
